...
header("Access-Control-Allow-Headers: X-Content-Only-Header, Content-Type");
...
if (!$network || !$type) {
    jsonError('network et type sont requis');
}
...
if (!$entry || !isset($entry['settings']['proxy']) || !is_array($entry['settings']['proxy'])) {
    jsonError('Paramètres proxy manquants pour le réseau: ' . $network, 500);
}

$proxySettings = $entry['settings']['proxy'];

function getProxyUrl($type, $settings) {
    if (isset($settings[$type]) && is_string($settings[$type]) && $settings[$type] !== '') {
...
$proxyUrl = getProxyUrl($type, $proxySettings);
if (!$proxyUrl) {
    jsonError('URL de proxy introuvable pour le type ' . $type, 500);
}
...
function checkAndCleanExpiredCache($cacheDir, $cacheMarkerFile, $cacheTTL) {
    ...
    if ($cacheAge > $cacheTTL) {
        $files = glob($cacheDir . '/*.{json,gz,zip}', GLOB_BRACE) ?: [];
        ...
        if (is_dir($shardDir)) {
            $shardFiles = glob($shardDir . '/*.json.gz') ?: [];
...
function proxyRaw($url) {
    $context = stream_context_create([
        'http' => [
            'method' => 'GET',
            'header' => "User-Agent: MyBusFinder Proxy/1.0\r\n"
        ]
    ]);

    $data = @file_get_contents($url, false, $context);
    if ($data === false) {
        jsonError('Impossible de récupérer la ressource distante', 502);
    }
...
function buildStopTimesShards($extractDir, $shardDir) {
    ...
    foreach ($shards as $shardKey => $data) {
        file_put_contents($shardDir . '/' . $shardKey . '.json.gz', gzencode(json_encode($data), 6));
    }
...
function extractAndOptimizeGTFS($zipCacheFile, $extractDir, $cacheDir, $coreCacheFile, $optimizedCoreFile, $optimizedRoutesFile, $optimizedStopsFile, $tripIndexFile) {
    $zip = new ZipArchive();
...
try {
    ...
    $info = [];
    $shardDir = $cacheDir . '/shards';
    $needsOptimization = !file_exists($optimizedCoreFile) || !file_exists($optimizedRoutesFile) || !file_exists($optimizedStopsFile) || !file_exists($shardDir . '/.built');

    if ($needsOptimization || $debug) {
        if (!file_exists($zipCacheFile) || (time() - filemtime($zipCacheFile) > $cacheTTL)) {
            $zipData = @file_get_contents($proxyUrl);
            if ($zipData === false) {
                throw new Exception('Impossible de télécharger le ZIP GTFS');
            }
    ...
    if (isset($_GET['action'])) {
        ...
        switch ($action) {
            ...
            case 'stop_times_by_trips':
                ...
                foreach ($requestedTrips as $tripId => $_) {
                    ...
                    if (!isset($shardCache[$shardKey]) && file_exists($shardFile)) {
                        $shardCache[$shardKey] = json_decode(gzdecode(file_get_contents($shardFile)), true);
                    }
            ...
            case 'route':
                ...
                while (($row = fgetcsv($fh)) !== false) {
                    if (($row[$routeIdIdx] ?? '') === $routeId) {
                        $tripId = $row[$tripIdIdx] ?? '';
                        $trips[] = ['trip_id' => $tripId, 'route_id' => $routeId, 'service_id' => $row[$serviceIdIdx] ?? ''];
                        $tripIds[$tripId] = true;
                    }
                }
                fclose($fh);
                ...
                $routeData = ['trips' => $trips, 'stopTimes' => $stopTimesByTrip];
            ...
            default:
                http_response_code(400);
                header('Content-Type: application/json');
                echo json_encode(['error' => 'Action invalide', 'action_received' => $action]);
                break;
        }
        exit;
    }
    ...
    if (isset($_SERVER['HTTP_X_CONTENT_ONLY_HEADER'])) {
        $context = stream_context_create(['http' => ['method' => 'GET', 'header' => 'Range: bytes=0-51200']]);
        $data = @file_get_contents($proxyUrl, false, $context);
    } else {
        $data = @file_get_contents($proxyUrl);
    }

    if ($data === false) {
        jsonError('Impossible de télécharger la ressource GTFS', 502);
    }
