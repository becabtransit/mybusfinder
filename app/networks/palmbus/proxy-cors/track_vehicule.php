<?php
/**
 * Cron script : met a jour vehicle_service.sqlite a partir du flux
 * GTFS-RT VehiclePosition relayer par proxy_vehpos.php
 *
 * A exécuter via cron, par ex toutes les minutes (cest l'idéal)
 *   * * * * * php /chemin/vers/update_vehicle_service.php >> /var/log/vehicle_service.log 2>&1
 */
...
function fetchGtfsRtData(string $proxyScript): string
{
    ...
    if ($data === false || $data === '') {
        throw new RuntimeException('proxy_vehpos.php a échoué');
    }
    return $data;
}
...
class ProtobufReader
{
    ...
    /** @return array{0:int,1:int} [numero de champ, wire type] */
...
try {
    ...
    $currentVehicles = decodeGtfsRtVehicles($raw); 
} catch (Throwable $e) {
    error_log('[mbh] échec récupération/décodage GTFSRT' . $e->getMessage());
    exit(1);
}

if (empty($currentVehicles)) {
    error_log('[mbh] aucun véhicule dans le flux GTFSRT');
    exit(0);
}
...
error_log(sprintf(
    '[mbh] Ok %d vehicules en service, %d passées hors service',
